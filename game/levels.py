...
def levelGen(LVL, player_pos, lengths, planes, sizes):
...
def changeLevel(player_pos, lengths, LEVELS, planes, sizes):
    total_length = 0
    ...
    if not levels[0]:
        levels[0] = 1
        levelGen(1, player_pos, lengths, planes, sizes)
    ...
    for i, n in enumerate(levels):
        if (passed_length < -player_pos) and (-player_pos < passed_length+lengths[i]) and (not n):
            levels[i] = 1
            levelGen(i+1, player_pos, lengths, planes, sizes)
            break
